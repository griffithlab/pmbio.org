...
### copyCat Somatic
copyCat is an R package used for detecting somatic (sample/control) copy number aberrations. It works by measuring the depth of coverage from a sequencing experiment. Because copyCat relies solely on depth of coverage for determining calls it works best with Whole Genome Sequencing. 
```R
```
