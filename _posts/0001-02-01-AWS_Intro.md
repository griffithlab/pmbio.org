...
![Image of a data center](/assets/module_1/DataCenter.jpg)
* * *
...
![AWS-Services](/assets/module_1/AWS-Services.png)
* * *
**The AWS EC2 dashboard:**
![AWS-EC2-Dashboard](/assets/module_1/AWS-EC2-Dashboard.png)
* * *
...
![AWS-EC2-PriceList](/assets/module_1/AWS-EC2-PriceList.png)
* * *
...
![AWS-EC2-LaunchInstance](/assets/module_1/AWS-EC2-LaunchInstance.png)
* * *
...
For this tutorial we will select the following AMI from the `Quick Start` list: `ami-0f65671a86f061fcd`. This number is a unique ID for the AMI. The full length description for this AMI is `Ubuntu Server 18.04 LTS (HVM), SSD Volume Type`. We are also told that the `Root device type` is `EBS` and the `Virtualization type` is `HVM`. `Ubuntu Server 18.04 LTS`, refers to the version of the Ubuntu OS. This version is also known as Ubuntu '[Bionic Beaver](https://en.wikipedia.org/wiki/Ubuntu_version_history#Ubuntu_18.04_LTS_(Bionic_Beaver))'. If you were going to install it directly onto your own hardware instead of on the AWS cloud you would download the [18.04 LTS image](http://releases.ubuntu.com/18.04/) from the Ubuntu releases site. We are told that the `Root device type` of the AMI is `EBS`. We will discuss storage in more detail but this means that the AMI is configured so that the `root volume` of the operating system will be installed on an EBS volume. In practical terms, this means that information stored on the root volume, including the OS itself will persist if we stop the instance (i.e. the root volume is *not* ephemeral). The term `HVM` refers to a type of virtualization technology that will be used by the instance, the other common type being `PV`. A detailed discussion of virtualizition technology is outside of the scope of this tutorial but you can learn more details here: [Linux AMI virtualization types](http://docs.aws.amazon.com/AWSEC2/latest/UserGuide/virtualization_types.html) and [Ubuntu PV vs HVM](http://stackoverflow.com/questions/22130214/amazon-ec2-ubuntupv-or-ubuntuhvm). On balance, the `HVM` option is now perhaps recommended over the `PV` option. Once you are ready to proceed, press the blue `Select` button next to the Ubuntu Server description.
...
![AWS-EC2-AddStorage2](/assets/module_1/AWS-EC2-AddStorage2.png)
* * *
...
![AWS-EC2-ConfigureSecurityGroup](/assets/module_1/AWS-EC2-ConfigureSecurityGroup.png)
* * *
...
![AWS-EC2-ReviewInstanceLaunch](/assets/module_1/AWS-EC2-ReviewInstanceLaunch.png)
* * *
...
![AWS-EC2-Dashboard2](/assets/module_1/AWS-EC2-Dashboard2.png)
* * *
...
![AWS-EC2-Login](/assets/module_1/AWS-EC2-Login.png)
* * *
...
sudo file -s /dev/xvdc | perl -ne 'chomp; if ($_ =~ /UUID\=(\S+)/){print "\nUUID=$1 /data ext4 defaults,nofail 0 2\n"}'
