...
We will be using data from a well-described breast cancer cell line (HCC1395) and its match lymphoblastoid cell line for subsequent analyses. The data are hosted on our server at [genomedata.org](http://genomedata.org/pmbio-workshop/fastqs/). 

The table below provides links and sample details for all samples. Note that the 'Readgroup ID' can be used to match downloaded files to these details.
